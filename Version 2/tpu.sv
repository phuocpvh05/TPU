...
/*
TPU architecture (top-level view)

Host / CPU
  |
  +--> Control Unit
  |      |-- sys_switch / ub_rd_start / ub_rd_transpose
  |      |-- ub_rd_col_size / ub_rd_row_size / ub_rd_addr / ub_ptr_select
  |      |-- vpu_data_pathway / learning_rate / inv_batch_size_times_two
  |
  +--> Unified Buffer (UB)
         |-- write host data / write-back data
         |-- read input matrix -> left side of systolic array
         |-- read weight matrix -> top side of systolic array
         |-- read bias / Y / H -> VPU
         |
         +--> Systolic Array (2x2)
                |-- PE11 -> PE12
                |-- PE21 -> PE22
                |-- MAC / accumulation pipeline
                |
                +--> VPU
                       |-- Bias
                       |-- Leaky ReLU
                       ...
                       |
                       +--> write results back to UB

Data flow:
  input/weight/bias -> UB -> systolic array -> VPU -> UB
*/
